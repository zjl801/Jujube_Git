...
public final class ScreenUtil {
    ...
    public static float getDensity() {
        return jUtils.getApp().getResources().getDisplayMetrics().density;
    }
    ...
    public static int getDensityDpi() {
        return jUtils.getApp().getResources().getDisplayMetrics().densityDpi;
    }
    ...
    public static int getStatusBarHeight() {
        int result = 0;
        int resourceId = jUtils.getApp().getResources().getIdentifier("status_bar_height", "dimen", "android");
        if (resourceId > 0) {
            result = jUtils.getApp().getResources().getDimensionPixelSize(resourceId);
        }
        return result;
    }
    ...
    public static int getNavBarHeight() {
        ...
        int resourceId = 0;
        // 判断底部导航栏是否显示
        int rid = jUtils.getApp().getResources().getIdentifier("config_showNavigationBar", "bool", "android");
        if (rid != 0) {
            resourceId = jUtils.getApp().getResources().getIdentifier("navigation_bar_height", "dimen", "android");
            if (resourceId > 0) {
                result = jUtils.getApp().getResources().getDimensionPixelSize(resourceId);
            }
        }
    ...
    public static boolean hasNavigationBar() {
        ...
        Resources rs = jUtils.getApp().getResources();
    ...
    public static boolean isLandscape() {
        return jUtils.getApp().getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE;
    }
    ...
    public static boolean isPortrait() {
        return jUtils.getApp().getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT;
    }
    ...
    public static boolean isScreenLock() {
        KeyguardManager km = (KeyguardManager) jUtils.getApp().getSystemService(Context.KEYGUARD_SERVICE);
    ...
    public static void setSleepTime(final int duration) {
        Settings.System.putInt(jUtils.getApp().getContentResolver(), Settings.System.SCREEN_OFF_TIMEOUT, duration);
    }
    ...
    public static int getSleepTime() {
        try {
            return Settings.System.getInt(jUtils.getApp().getContentResolver(), Settings.System.SCREEN_OFF_TIMEOUT);
    ...
    public static boolean isTablet() {
        return (jUtils.getApp().getResources().getConfiguration().screenLayout
