 ...
 * Describe:
 */
public class BGIPhotoFolderPw extends BGIBasePopupWindow implements BGAOnRVItemClickListener {
    ...
    public BGIPhotoFolderPw(Activity activity, View anchorView, Delegate delegate) {
     ...
     * @param data
     */
    public void setData(ArrayList<BGIPhotoFolderModel> data) {
    ...
    public void show() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.N) {
                setHeight(BGIPhotoPickerUtil.getScreenHeight() - offsetY);
            }
    ...
    public void dismiss() {
        ...
        mContentRv.postDelayed(new Runnable() {
            @Override
            public void run() {
                BGIPhotoFolderPw.super.dismiss();
            }
        }, ANIM_DURATION);
    }
    ...
    private class FolderAdapter extends BGARecyclerViewAdapter<BGIPhotoFolderModel> {
        ...
        public FolderAdapter(RecyclerView recyclerView) {
            ...
            mData = new ArrayList<>();
            mImageSize = BGIPhotoPickerUtil.getScreenWidth() / 10;
        }

        @Override
        protected void fillData(BGAViewHolderHelper helper, int position, BGIPhotoFolderModel model) {
            ...
            BGIImage.display(helper.getImageView(R.id.iv_item_photo_folder_photo), R.mipmap.bga_pp_ic_holder_light, model.coverPath, mImageSize);
        }
    }
